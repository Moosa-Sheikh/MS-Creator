    ...
    <SidebarProvider>
      <div className="flex h-screen w-full bg-background overflow-hidden">
        <Sidebar variant="inset" className="border-r border-sidebar-border bg-sidebar">
          <SidebarHeader className="p-4">
            <h2 className="text-lg font-bold text-sidebar-foreground flex items-center gap-2">
              <span className="w-6 h-6 rounded bg-sidebar-primary flex items-center justify-center text-sidebar-primary-foreground text-xs">
                M
              </span>
              Mockup Studio
            </h2>
          ...
          <SidebarContent>
            <SidebarMenu>
              <SidebarMenuItem>
                ...
                {activeProduct && (
                  <p className="px-3 pb-1 text-[11px] text-sidebar-foreground/50 truncate leading-none">
                    {activeProduct.name}
                  </p>
                )}
              </SidebarMenuItem>
        ...
        <main className="flex-1 h-full overflow-y-auto relative">
          ...
          />
          <div className="relative z-10 h-full p-8 max-w-6xl mx-auto">{children}</div>
        </main>
      </div>
    </SidebarProvider>
  );
